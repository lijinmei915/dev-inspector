...
- 普通 DOM 容器：选中重复集合、列表网格或普通列表外层时，也按当前外层元素处理；不展示子项摘要或布局容器摘要，只展示外层属性；里层卡片、标签和文字需要单独选中后再改。
...
- Library / 设计库：面板顶部固定轻入口，支持打开 Admin Shell 全开管理页；Tokens / Components / Changes / Usage 可统一查看、搜索、筛选、详情、复制、修改篮复核和使用治理，并以受控草稿方式新增、编辑和删除，最后进入 `本次修改内容` / `发送给AI` 链路。Components 在全开页按分类优先展示宿主通过 `componentPreviews` 注入的真实组件预览卡片，并覆盖能力表里已登记的变体、状态、颜色和尺寸组合；中间真实组件实例支持规格级选中，右侧按具体规格显示用途、能力项、props、selector、token 绑定和业务页面使用次数。
...
- 已完成：demo 引用 workspace source；文字和容器样式完成预设 / 自定义双态；负数间距输入；相同元素 primitive 规则；发送给 AI 短结构文本；组件模式默认面板第一版；Button / Icon / Badge / Card 已进入组件能力表；Card 已收敛为真实变体并支持标题/说明文内容插槽；面板主按钮视觉已收敛为单主按钮；组件命名规范已沉淀为 source selector / display name / attributes 三层；普通元素 `创建组件` 抽屉已接入本次修改篮；页面壳层、重复集合和列表网格都已改为普通外层样式模块；顶部 `Library / 设计库` 已接入 Tokens 统一查看、Components 真实组件 registry 预览、规格级选中详情、Changes 修改篮和 Usage 使用治理的 Admin Shell 全开管理页。
