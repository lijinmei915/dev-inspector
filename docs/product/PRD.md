...
- 外部项目接入采用轻声明 + 自动识别 + 人工确认映射的三段式方案。
- 先由外部项目声明组件类型、变体、尺寸、状态和 token 名，再由 Inspector 按 selector / class / DOM 结构 / computed style 自动识别。
- 识别成功后统一映射成标准展示名和 token 语义；识别不到的值先作为自定义值或待确认映射，不静默猜测。
- 该方向当前只作为后续接入策略，不进入本阶段实现。

### 接入协议草案

| 层级 | 外部项目提供 | Inspector 负责 | 备注 |
|------|--------------|---------------|------|
| 组件声明 | 组件类型、推荐 selector、变体、尺寸、状态、可编辑内容 | 读取声明并建立候选组件目录 | 轻声明，允许缺项 |
| Token 声明 | token 名、值、用途、可选别名 | 把页面真实值映射到 token 语义 | 同一值可命中多个别名时，以标准名为准 |
| 识别匹配 | 页面 DOM、class、data-*、computed style | 生成组件 / 变体 / token 的识别结果 | 置信度不足时进入待确认 |
| 人工确认 | 由用户确认命名或映射是否正确 | 写入本地映射缓存或修改篮 | 先能用，再逐步变准 |

#### 声明优先级

1. 显式 `data-component` / `componentPreviews` registry
2. 外部项目声明文件
3. 现有 class / selector / DOM 结构推断
4. computed style 与 token 反查

#### 需要人工确认的情况

- 同一个页面值可能对应多个 token。
- 组件变体只能从样式推断，无法稳定落到唯一规格。
- selector 能命中，但 DOM 结构与声明不一致。
- 识别结果会影响 `删除 token`、`替换组件` 或 `新增变体` 这类破坏性动作。
